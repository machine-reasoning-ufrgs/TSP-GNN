...
def build_network(d):
    ...
    edge_initial_embeddings = edge_init_MLP(edge_weight)
    ...
    GNN['predictions'] = E_prob

    # Compute the number of problems in the batch
    num_problems = tf.shape(n_vertices)[0]

    # Compute a predicted optimal route cost for each problem
    pred_costs = tf.while_loop(
        lambda i, pred_costs: tf.less(i, num_problems),
        lambda i, pred_costs:
            (
                (i+1),
                pred_costs.write(
                    ...
                    tf.reduce_sum(
                        tf.multiply(
                            tf.reshape(edge_weight[tf.reduce_sum(n_edges[0:i]):tf.reduce_sum(n_edges[0:i])+n_edges[i]], [-1]),
                            E_prob[tf.reduce_sum(n_edges[0:i]):tf.reduce_sum(n_edges[0:i])+n_edges[i]],
                        )
                    )
                )
            ),
    ...
    # Compute deviations from ground truth optimal costs
    GNN['deviations'] = tf.div(tf.subtract(pred_costs, target_cost), target_cost)    

    # Define loss
    GNN['loss'] = tf.reduce_mean(tf.square(GNN['deviations']))
